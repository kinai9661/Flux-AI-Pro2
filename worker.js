...
const CONFIG = {
    ...
    HD_OPTIMIZATION: {
      ...
      QUALITY_MODES: {
        ...
        ultra_4k: { name: "4K超高清", description: "4K 專用模式", min_resolution: 2048, max_resolution: 4096, steps_multiplier: 1.5, guidance_multiplier: 1.2, hd_level: "ultra_4k", force_upscale: true }
      },
      HD_PROMPTS: {
  ...
  class PollinationsProvider {
      constructor(config, env) {
          ...
          this.env = env;
          this.apiKey = env?.POLLINATIONS_API_KEY || null;
      }
          ...
          for (const tryModel of modelsToTry) {
              for (let retry = 0; retry < CONFIG.MAX_RETRIES; retry++) {
                  try {
                      ...
                      let url = this.config.endpoint + "/image/" + encodedPrompt + "?" + params.toString();
                      ...
                      if (retry < CONFIG.MAX_RETRIES - 1) {
                          ...
                          logger.add("🔄 Retrying", { delay: delay + 'ms', nextRetry: retry + 2 });
