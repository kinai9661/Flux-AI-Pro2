...
const CONFIG = {
    ...
    PROVIDERS: {
        ...
        models: [
          ...
          { id: "flux-kontext", name: "Flux Kontext 🎨", confirmed: true, category: "kontext", description: "圖像編輯 (1張參考圖)", max_size: 2048, supports_reference_images: true, max_reference_images: 1 },
          { id: "flux-kontext-pro", name: "Flux Kontext Pro 💎", confirmed: true, category: "kontext", description: "圖像編輯專業版 (1張參考圖)", max_size: 2048, supports_reference_images: true, max_reference_images: 1 }
        ],
        rate_limit: null,
    ...
    HD_OPTIMIZATION: {
      enabled: true,
      QUALITY_MODES: {
        economy: { name: "經濟模式", description: "快速出圖,適合測試", min_resolution: 512, max_resolution: 1600, steps_multiplier: 0.85, guidance_multiplier: 0.9, hd_level: "basic" },
        standard: { name: "標準模式", description: "平衡質量與速度", min_resolution: 1024, max_resolution: 2048, steps_multiplier: 1.0, guidance_multiplier: 1.0, hd_level: "enhanced" },
        ultra: { name: "超高清模式", description: "極致質量,耗時較長", min_resolution: 1280, max_resolution: 4096, steps_multiplier: 1.35, guidance_multiplier: 1.15, hd_level: "maximum", force_upscale: true },
  ...
  async function fetchPollinationsModels(env) {
      const cacheKey = 'pollinations_models';
      const cached = apiCache.get(cacheKey);
      if (cached) return cached;
      const headers = { 'Accept': 'application/json', 'Referer': 'https://gen.pollinations.ai/' };
      if (env && env.POLLINATIONS_API_KEY) {
          headers['Authorization'] = 'Bearer ' + env.POLLINATIONS_API_KEY;
      }
      let filtered = [];
      try {
          const res = await fetchWithTimeout('https://gen.pollinations.ai/image/models', { method: 'GET', headers }, 20000);
          if (res.ok) {
              const data = await res.json();
              let list = [];
              if (Array.isArray(data)) {
                  list = data.map(m => typeof m === 'string' ? { id: m, name: m } : { id: m.id || m.name || m, name: m.name || m.id || m });
              } else if (data && Array.isArray(data.models)) {
                  list = data.models.map(m => typeof m === 'string' ? { id: m, name: m } : { id: m.id || m.name || m, name: m.name || m.id || m });
              }
              filtered = list.filter(m => {
                  const s = (m.id || m.name || '').toLowerCase();
                  return s.includes('flux') || s.includes('kontext') || s.includes('zimage') || s.includes('z-image');
              }).map(m => {
                  const s = (m.id || '').toLowerCase();
                  const supports_reference_images = s.includes('kontext');
                  const max_reference_images = supports_reference_images ? 1 : 0;
                  const category = s.includes('kontext') ? 'kontext' : (s.includes('zimage') || s.includes('z-image') ? 'zimage' : 'flux');
                  return { id: m.id, name: m.name, provider: 'pollinations', category, description: '', max_size: 2048, confirmed: true, experimental: false, fallback: null, ultra_hd: false, supports_reference_images, max_reference_images };
              });
          }
      } catch (_) {
          filtered = (CONFIG.PROVIDERS.pollinations.models || []).filter(m => {
              const s = (m.id || '').toLowerCase();
              return s.includes('flux') || s.includes('kontext') || s.includes('zimage') || s.includes('z-image');
          }).map(m => ({ id: m.id, name: m.name, provider: 'pollinations', category: m.category || 'flux', description: m.description || '', max_size: m.max_size || 2048, confirmed: m.confirmed !== false, experimental: m.experimental === true, fallback: m.fallback || null, ultra_hd: m.ultra_hd || false, supports_reference_images: m.supports_reference_images || false, max_reference_images: m.max_reference_images || 0 }));
      }
      apiCache.set(cacheKey, filtered, 300);
      return filtered;
  }
        ...
        async generate(prompt, options, logger) {
          ...
          if (referenceImages && referenceImages.length > 0) {
              if (!supportsRefImages) {
                  logger.add("⚠️ Reference Images", { 
                      ...
                      supported_models: ["flux-kontext", "flux-kontext-pro"] 
                  });
          ...
          for (const tryModel of modelsToTry) {
              for (let retry = 0; retry < CONFIG.MAX_RETRIES; retry++) {
                  try {
                      if (this.config.requires_key && !this.apiKey) {
                          throw new Error("POLLINATIONS_API_KEY 未配置，請在環境變量中設置");
                      }
                      let url = this.config.endpoint + "/image/" + encodedPrompt;
      ...
      try {
          ...
          if (options.seed !== -1 && referenceImages.length === 0 && options.numOutputs === 1) {
              ...
              if (cachedResult) {
                  ...
                  const cachedWithProxy = cachedResult.map(r => ({
                      ...r,
                      url: '/proxy-image/' + encodeURIComponent(r.original_url || r.url),
                      original_url: r.original_url || r.url
                  }));
                  return new Response(JSON.stringify({
                      created: Math.floor(Date.now() / 1000),
                      data: cachedWithProxy,
                      cached: true,
                      cache_key: cacheKey
                  }), { 
                      headers: corsHeaders({ 
          ...
          return new Response(JSON.stringify({ 
              created: Math.floor(Date.now() / 1000), 
              data: results.map(r => ({ 
                  url: '/proxy-image/' + encodeURIComponent(r.url), 
                  original_url: r.url,
                  provider: r.provider, 
                  model: r.model, 
                  seed: r.seed, 
  ...
  async function handleModelsRequest(env) {
      const models = await fetchPollinationsModels(env);
  ...
  async function handleImageProxy(request, env, ctx) {
      try {
          ...
          const imageUrl = decodeURIComponent(url.pathname.replace('/proxy-image/', ''));
          ...
          console.log('✅ Image proxy successful:', imageUrl);
          
          const responseHeaders = corsHeaders({
              'Content-Type': contentType,
              'Cache-Control': 'public, max-age=3600',
              'X-Proxy-Source': 'pollinations-api'
          });
      ...
      async fetch(request, env, ctx) {
          ...
          try {
              ...
              if (url.pathname === '/') {
                  ...
                  response = await handleModelsRequest(env);
                  ...
                  response = await handleImageProxy(request, env, ctx);
              } else {
                  response = new Response(JSON.stringify({
  ...
  *{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;background:linear-gradient(135deg,#0a0a0a 0%,#1a1a2e 100%);color:#fff;padding:20px;min-height:100vh}.container{max-width:1400px;margin:0 auto}
  ...
  <small id="refImageLimit" style="color:#9ca3af;font-size:11px">kontext: 最多1張</small>
  </div>
  
  <label>AI 模型</label>
  <select id="model" onchange="updateRefImageLimit()"></select>
  ...
  <optgroup label="⬜ 方形系列">
  <option value="square-512" selected>方形 512px (快速測試)</option>
  <option value="square-1k">方形 1K (標準)</option>
  ...
  <label>寬度: <span id="widthValue">768</span>px</label>
  <input type="range" id="width" min="256" max="4096" step="64" value="768">
  <label>高度: <span id="heightValue">768</span>px</label>
  <input type="range" id="height" min="256" max="4096" step="64" value="768">
  ...
  <option value="economy" selected>⚡ 經濟</option>
  ...
  const data=await res.json();
  MODELS=(data.data||[]).filter(x=>x.provider==='pollinations');
  const select=document.getElementById('model');
  select.innerHTML='';
  const groups=[{label:'⚡ Flux 系列',key:'flux'},{label:'🎨 圖像編輯',key:'kontext'},{label:'🖼️ Z-Image 系列',key:'zimage'}];
  for(const g of groups){
  const og=document.createElement('optgroup');
  og.label=g.label;
  MODELS.filter(m=>{
  const cat=m.category||(m.id||'').includes('kontext')?'kontext':((m.id||'').includes('zimage')||(m.id||'').includes('z-image')?'zimage':'flux');
  return cat===g.key;
  }).forEach(m=>{
  ...
  function updateSizeForQuality(){
  const mode=document.getElementById('qualityMode').value;
  if(mode==='economy'){setSize(768,768);}
  else if(mode==='standard'){setSize(1024,1024);}
  else if(mode==='ultra'){setSize(1536,1536);}
  else if(mode==='ultra_4k'){setSize(2048,2048);}
  }
  ...
  function refreshImage(button, imageUrl) {
    ...
    loadingDiv.innerHTML = '<div style="font-size:24px;margin-bottom:10px">🔄</div><div>手動刷新中...</div><div style="font-size:12px;margin-top:5px;opacity:0.7;">使用代理請求</div>';
    ...
    imgElement.onerror = function() {
      ...
      // 嘗試獲取原始URL
      const originalUrl = imageUrl.includes('/proxy-image/') ? 
        decodeURIComponent(imageUrl.replace('/proxy-image/', '')) : imageUrl;
      loadingDiv.innerHTML = '<div style="color:#ef4444;font-size:20px;margin-bottom:10px">❌</div><div style="color:#ef4444;">刷新失敗</div><button onclick="window.open(\'' + originalUrl + '\')" style="margin-top:8px;padding:4px 8px;font-size:11px;background:rgba(139,92,246,0.3);border:1px solid #8b5cf6;color:#fff;border-radius:4px;cursor:pointer">🔗 直接訪問原圖</button>';
      imgElement.style.display = 'none';
    };
  ...
  document.getElementById('qualityMode').addEventListener('change',updateSizeForQuality);
  ...
  imgElement.onclick = function(){window.open(item.original_url || item.url,'_blank');};
  ...
  // 圖片加載事件 - 實現 Solution 1 (延遲加載 + 重試) 和 Solution 3 (代理請求)
  let retryCount = 0;
  const maxRetries = 6;
  const baseRetryDelay = 2000; // 基礎延遲2秒
  const initialDelay = 3000; // 初始延遲3秒 (Solution 1)
  ...
  function loadImageWithRetry() {
    ...
    imgElement.onerror = function(){
      ...
      if (retryCount < maxRetries) {
        const nextDelay = baseRetryDelay * Math.pow(1.5, retryCount - 1); // 指數退避
        loadingDiv.innerHTML = '<div style="font-size:24px;margin-bottom:10px">🔄</div><div>圖片準備中，重試 ' + retryCount + '/' + maxRetries + '</div><div style="font-size:12px;margin-top:5px;opacity:0.7;">使用代理請求，' + Math.round(nextDelay/1000) + '秒後重試...</div>';
        ...
        loadingDiv.innerHTML = '<div style="color:#ef4444;font-size:20px;margin-bottom:10px">❌</div><div style="color:#ef4444;">圖片加載失敗</div><div style="font-size:12px;margin-top:8px;color:#9ca3af;">已重試 ' + maxRetries + ' 次</div><button onclick="window.open(\'' + originalUrl + '\')" style="margin-top:8px;padding:4px 8px;font-size:11px;background:rgba(139,92,246,0.3);border:1px solid #8b5cf6;color:#fff;border-radius:4px;cursor:pointer">🔗 直接訪問原圖</button>';
        imgElement.style.display = 'none';
      }
    };
    
    // Solution 1: 初始延遲3秒開始加載，給API時間處理
    setTimeout(() => {
      console.log('🕐 延遲', initialDelay/1000, '秒後開始加載圖片:', item.url);
      loadingDiv.innerHTML = '<div style="font-size:24px;margin-bottom:10px">⏳</div><div>開始加載圖片...</div><div style="font-size:12px;margin-top:5px;opacity:0.7;">使用代理請求提高成功率</div>';
  ...
  addToHistory({
  url:item.original_url || item.url, // 存儲原始URL到歷史記錄
